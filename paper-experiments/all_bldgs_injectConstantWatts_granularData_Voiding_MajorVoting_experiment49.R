...
Yc = Y[((OFFSET+8)*720):((OFFSET+14)*720), c(1:n.bldgs)] 
...
for (mag in c(10, 20, 30, 40, 50)){ # Injections are in KWs
  for (end in sample(100:700, 200, replace=TRUE)){
    ...
    Yc = Y[((OFFSET+8)*720):((OFFSET+14)*720), CLUSTER] 
    ...
    capture.output(m_VAR <- model_selection(Ycc, m1$bic, maxp,  log.file = paste(c('./experiment49-logs/var_model_for_bldg_', ANOMALOUS_BLDG,'.log'), collapse = '')), file='/dev/null')
