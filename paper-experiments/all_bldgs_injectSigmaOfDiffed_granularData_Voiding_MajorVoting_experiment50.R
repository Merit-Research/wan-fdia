...
for (mag in c(1, 2, 3, 4, 5)){ # Injections are in sigma's
  for (end in sample(100:700, 200, replace=TRUE)){
    ...
    capture.output(m_VAR <- model_selection(Ycc, m1$bic, maxp,  log.file = paste(c('./experiment50-logs/var_model_for_bldg_', ANOMALOUS_BLDG,'.log'), collapse = '')), file='/dev/null')
